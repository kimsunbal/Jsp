						...
						<article class="blog_style1">
							<div class="blog_img">
								<img class="img-fluid" src="${board.previewImg}" alt=""
									style="width: 100%; max-height: 269.347px">
							</div>
							<div class="blog_text">
								<div class="blog_text_inner">
									<div class="cat">
										<a class="cat_btn" href="#">${board.user.username}</a> <a
											href="#"><i class="fa fa-calendar" aria-hidden="true"></i>
											${board.createDate}</a> <a href="#"><i
									...
									<a class="blog_btn"
										href="/blog/board?cmd=detail&id=${board.id}">Read More</a>
								</div>
							</div>
						</article>
						<!-- 블로그끝 -->
						...
						<ul class="pagination">
								...
								<c:choose>
									<c:when test="${param.page>5}">
										<li class="page-item"><a
											href="/blog/board?cmd=search&content=${searchContent}&page=${param.page-5}"
									...
									<c:otherwise>
										<li class="page-item"><a
											href="/blog/board?cmd=search&content=${searchContent}&page=1"
											class="page-link" aria-label="Previous"> <span
												aria-hidden="true"> <span
													class="lnr lnr-chevron-left"></span>
											</span>
							...
							<c:if test="${param.page==count}">
								<c:if test="${param.page-4>0}">
									<li class="page-item"><a
										href="/blog/board?cmd=search&content=${searchContent}&page=${param.page-4}"
										class="page-link">${param.page-4}</a></li>
								</c:if>
							</c:if>
							<c:if test="${param.page>=count-1}">
								<c:if test="${param.page-3>0}">
									<li class="page-item"><a
										href="/blog/board?cmd=search&content=${searchContent}&page=${param.page-3}"
										class="page-link">${param.page-3}</a></li>
								</c:if>
							</c:if>
							<c:if test="${param.page>2}">
								<c:if test="${param.page-2>0}">
									<li class="page-item"><a
										href="/blog/board?cmd=search&content=${searchContent}&page=${param.page-2}"
										class="page-link">${param.page-2}</a></li>
								</c:if>
							</c:if>
							<c:if test="${param.page>1}">
								<c:if test="${param.page-1>0}">
									<li class="page-item"><a
										href="/blog/board?cmd=search&content=${searchContent}&page=${param.page-1}"
										class="page-link">${param.page-1}</a></li>
								</c:if>
							</c:if>

							<li class="page-item active"><a
								href="/blog/board?cmd=search&content=${searchContent}&page=${param.page}"
								class="page-link">${param.page}</a></li>
					...
					<aside class="single_sidebar_widget search_widget">
						<div class="input-group">
							<input type="text" class="form-control"
								placeholder="Search Posts" id="searchText"> <span
					...
					<aside class="single_sidebar_widget popular_post_widget">
						...
						<c:forEach var="board" items="${hotBoards}">
							<div class="media post_item">
								<img src="${board.previewImg}" width="100" height="80"
									alt="post">
